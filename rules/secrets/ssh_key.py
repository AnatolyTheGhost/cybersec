"""
Rule to detect SSH private keys.
"""
...
class SshKeyRule(BaseRule):
    """
    Rule to detect embedded OpenSSH private keys (-----BEGIN OPENSSH PRIVATE KEY-----).
    """

    id = "secrets.ssh_key"
    name = "Embedded SSH Private Key"
    description = "Detects embedded OpenSSH private key blocks."
    ...
    def analyze(self, context: Any) -> list[Finding]:
        ...
        for literal in iter_literals(semantic_ast):
            ...
            if regex_search(r"-----BEGIN OPENSSH PRIVATE KEY-----.*-----END OPENSSH PRIVATE KEY-----", normalized):
                report(literal, "Embedded OpenSSH private key detected.")
